...
### 1. 教育コンテンツの提案（教育サイト向け）

`config/learner-context.yaml` を参照し、子供の学習状況に合わせた提案を行う。

**提案の流れ:**

1. 現在の学年・学習進度を確認
2. 学習指導要領や教育研究を `web_search` で調査
3. 子供が次に伸ばすべきスキルを特定
4. そのスキルを伸ばすプリントの仕様を考える
5. 既存の問題タイプでカバーできるか、新モードが必要か判断
6. Feature Issue を作成（教育的背景 + 技術的実装案の両方を含める）

**Issue の Problem / Why に含める教育的背景:**

- なぜこの種類のプリントが必要か（発達段階、学習指導要領の根拠）
- どんなスキルが伸びるか（計算速度、漢字の定着、読解力等）
- 類似の教材・アプリでの実装例（web_search で調査）
- 対象年齢と前提知識

**例: kanji-practice への提案**

- 「熟語の読み」モード → 3年生以降、文脈での漢字理解が重要になる
- 「送りがなの使い分け」モード → 間違えやすい送りがなのドリル
- 「同じ部首の漢字」モード → 部首への意識を育てる

**例: math-worksheet への提案**

- 「時計の読み」問題 → 1-2年生でつまずきやすい単元
- 「文章問題（日本語）」の充実 → 立式力を鍛える
- 「虫食い算」の難易度バリエーション → 逆算思考の練習
- 「概算・見積もり」問題 → 暗算力と数の感覚を育てる

**例: english-note-maker への提案**

- 「フォニックス練習」モード → 音と文字の対応を学ぶ
- 「短文の書き写し」モード → 文構造の定着
- 「曜日・月・数字」の練習シート → 基本語彙の反復

### 2. KB ベースの技術提案

- `knowledge/repos/{name}-kb.md` の技術スタックから改善機会を検出
- テストカバレッジが低い → テスト追加の Feature Issue
- 手動デプロイ → CI/CD 自動化の Feature Issue
- 古い依存関係 → モダン化の提案

### 3. 競合・トレンドリサーチ

- `web_search` で類似ツールの最新機能を調査
- 教育ツール: 他の漢字ドリル・計算プリントアプリの機能
- MCP server: MCP SDK の最新バージョン対応
