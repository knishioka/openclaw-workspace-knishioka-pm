...
- `playbook_version`: 当時の `docs/codex-playbook.md` 先頭の `<!-- version: YYYY-MM-DD -->` から抽出した値。`scripts/codex-resolve.sh` が起動時に stderr へ `[codex-resolve] playbook_version=...` を出力するので cron 側はそれを capture する。
- `lint_available`: lint コマンドが対象リポで解決できたか (`true` / `false`)。判定 heuristic は `command -v {tool} >/dev/null` または `npm run lint` の `Missing script` 検出。
- `lint_skipped_reason`: `lint_available=false` のときのみ設定。`"no_lint_configured"` / `"command_not_found"` / `null`。

JSONL は append-only。新フィールドは新しい記録から適用し、過去レコードへ遡及追加しない。
...
- Quality Score 分布を `playbook_version` 別に層別表示する (markdown table で十分)。`playbook_version` が欠けている古いレコードは `unknown` として 1 行に集約する。
- **playbook 変更前後の Quality Score 比較**: 直近 2 つの playbook_version を取り、それぞれの A/B/C/D 比率と平均 days_to_resolve を並べる。改善 (A 比率↑) / 劣化 (C+D 比率↑) を矢印で示し、playbook 改訂のフィードバックループを回す。サンプル数が片側 < 5 件のときは「サンプル不足」と注記して比較を保留する。
- `lint_available == false` が **2 ヶ月以上連続するリポ** はフラグ表示し、reports/monthly-portfolio-{YYYY-MM}.md の "Tech Health" セクションで Linter 導入を別 Issue 候補として列挙する (このレビューで自動 Issue 作成はしない、manual 判断)。
