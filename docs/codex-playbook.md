...
> ただし **Lint が未導入で実行できない場合は行を省略しない** — `⚠️ n/a (Linter未導入)` で残す
> （理由は「Lint 不在リポの扱い」節を参照）。

例: lint 未導入のリポでの動作確認テーブル抜粋

| 項目      | コマンド        | 結果                  |
| --------- | --------------- | --------------------- |
| Build     | `npm run build` | ✅ pass               |
| Lint      | -               | ⚠️ n/a (Linter未導入) |
| Unit test | `npm test`      | ✅ 12 passed          |
...
監視リポ側で AGENTS.md / `.github/PULL_REQUEST_TEMPLATE.md` / `scripts/verify.sh` を新規整備するときは、
workspace 側の `docs/templates/` をスケルトンとして使う:

- `docs/templates/agents-md-template.md` — AGENTS.md (8 セクション固定、≤100 行目安)
- `docs/templates/pull-request-template.md` — `.github/PULL_REQUEST_TEMPLATE.md` (≤50 行目安)
- `docs/templates/verify-sh-template.sh` — `scripts/verify.sh` (`--json` 対応、exit 0/1/2)

テンプレは「埋めるべき項目」を揃えるためのものであり、プロファイル化はしない。
リポ毎の差分 (言語・スタック・固有制約) はテンプレに沿って各リポで独立にカスタマイズする。
